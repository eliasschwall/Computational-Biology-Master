---
title: "CB_Seminar_Homework_Beyer_03_Elias_Schwall"
...
```{r warning=FALSE}
# Adding tissue column to Expression_Pancreas_Lung_1000genes
Expression_Pancreas_Lung_1000genes$tissue <- Annotation$tissue
...
for (i in 1:1000){
  model <- glm(tissue ~ Expression_Pancreas_Lung_1000genes[,i], 
               data = Expression_Pancreas_Lung_1000genes, 
               family = "binomial")
  
 p_values[i] <- summary(model)$coefficients[, "Pr(>|z|)"]
}

# How many genes are predictive of tissue, at a significance threshold of 0.05?
p_values <- dplyr::filter(as.data.frame(p_values), p_values < 0.05)
```
There are `r nrow(p_values)` genes predictive of tissue at a significance threshold of 0.05.
